# /raid/vladimir_albrekht/anaconda/envs/conaiki_qwen_omni/lib/python3.10/site-packages/transformers/models/qwen2_5_omni/modeling_qwen2_5_omni.py
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
@dataclass
class Qwen2_5OmniThinkerCausalLMOutputWithPastAndGate(Qwen2_5OmniThinkerCausalLMOutputWithPast):
    gate_logits: Optional[torch.FloatTensor] = None  # [B, C]
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        # --- Conaiki additions (lightweight) ---
        d_model = config.text_config.hidden_size
        # NEW: small 3-way classifier on top of pooled LM hidden state
        num_gate_classes = getattr(config, "gate_num_classes", 3)
        self.conaiki_gate = nn.Sequential(
            nn.Linear(d_model, 2 * d_model, bias=False),
            nn.SiLU(),
            nn.Linear(2 * d_model, num_gate_classes, bias=False),
        )
        # (optional) hyperparam to weight gate loss
        self.config.gate_loss_weight = getattr(config, "gate_loss_weight", 1.0)

        self.conaiki_time = nn.Linear(16, d_model)      # Fourier time → d_model        # ---------------------------------------
        self.post_init()

     # ---- Conaiki helpers ----------------------------------------------------
    @staticmethod
    def _fourier_time(t: torch.Tensor) -> torch.Tensor:
        """
        t: [B, T] or [T] seconds (float).
        returns: [B, T, 16] or [T, 16] fixed Fourier features.
        """
        if t.dim() == 1: t = t[None, :]  # [1, T]
        device = t.device
        freqs = torch.tensor([1,2,4,8,16,32,64,128.0], device=device)
        x = t[..., None] * (2*math.pi*freqs)  # [B,T,8]
        return torch.cat([torch.sin(x), torch.cos(x)], dim=-1)  # [B,T,16]

    def conaiki_align_modules(self):
        ref = self.get_input_embeddings().weight
        dev, dt = ref.device, ref.dtype
        self.conaiki_time.to(dev, dtype=dt)
        self.conaiki_gate.to(dev, dtype=dt)


    def _pool_from_audio_tokens(self, hidden_states: torch.Tensor, input_ids: torch.LongTensor):
        B, T, D = hidden_states.shape
        device = hidden_states.device
        
        audio_idx = self.config.audio_token_index
        audio_mask = (input_ids == audio_idx)
        
        # Debug
        #print(f"Audio tokens found: {audio_mask.sum(dim=1).tolist()}")
        
        has_audio = audio_mask.any(dim=1)
        
        if not has_audio.any():
            print("WARNING: No audio tokens found, using last position")
            return hidden_states[:, -1, :]

        # default: last non-pad index
        # (works if attention_mask exists; else just T-1)
        last_idx = torch.full((B,), T-1, device=device, dtype=torch.long)

        # where audio exists: take last audio position
        if has_audio.any():
            # turn each row into indices; take last True
            # find last True: argmax on reversed mask
            rev = torch.flip(audio_mask.to(torch.int8), dims=[1])             # [B,T]
            last_from_end = torch.argmax(rev, dim=1)                           # [B]
            last_audio_idx = (T - 1) - last_from_end                           # [B]
            last_idx = torch.where(has_audio, last_audio_idx, last_idx)        # [B]

        gather_idx = last_idx.view(B, 1, 1).expand(B, 1, D)                    # [B,1,D]
        pooled = hidden_states.gather(dim=1, index=gather_idx).squeeze(1)      # [B,D]
        return pooled
                                # [B,D]
    ...
    @replace_return_docstrings(
        output_type=Qwen2_5OmniThinkerCausalLMOutputWithPastAndGate,
        config_class="Qwen2_5OmniThinkerConfig",
    )
    def forward(
        self,
        ...
        # ---- Conaiki streaming path ----
        gate_labels: Optional[torch.LongTensor] = None,
        return_gate_logits: Optional[bool] = False,

        # --------------------------------
    ) -> Union[Tuple, Qwen2_5OmniThinkerCausalLMOutputWithPastAndGate]:
        r"""
        Args:
        ...
        # 2. Merge text , audios , image and video (original path)
        ...
        if labels is not None:
            ...
            if valid_labels > 0:
                ...
                # No valid labels - Phase A case
                loss = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)

        # --- NEW: gate head ---
        gate_logits = None
        gate_loss = None
        if return_gate_logits or (gate_labels is not None):
            #print(f"Computing gate: gate_labels={gate_labels}, input_ids.shape={input_ids.shape}")
            pooled = self._pool_from_audio_tokens(hidden_states, input_ids)
            #print(f"Pooled shape: {pooled.shape}")
            gate_logits = self.conaiki_gate(pooled)
            #print(f"Gate logits: {gate_logits}")
            
            if gate_labels is not None:
                gate_loss = torch.nn.functional.cross_entropy(gate_logits.float(), gate_labels)
                print(f"Gate loss: {gate_loss.item()}")
                w = float(getattr(self.config, "gate_loss_weight", 1.0))
                
                # Fix: Check if loss exists and is effectively zero
                if loss is None:
                    loss = w * gate_loss
                elif loss.abs().mean() < 1e-6:  # Phase A case
                    loss = w * gate_loss
                    print(f"Phase A detected, using only gate loss: {loss.item()}")
                else:
                    loss = loss + w * gate_loss
                    print(f"Phase B, combined loss: {loss.item()}")

        if not return_dict:
            # tuple path: put gate_logits at the end if requested
        ...
        # return dict path
        return Qwen2_5OmniThinkerCausalLMOutputWithPastAndGate(
            loss=loss,
            logits=logits,
